...
x = np.linspace(start=sample_mean - 3 * sample_std_dev, stop=sample_mean + 3 * sample_std_dev, num=10000)
...
(fit_loc, fit_scale) = expon.fit(exponential_samples, floc=0)
...
# Hint: Remember the functions described in the examples, choose an appropriate range for x
exp_x = np.linspace(start=0, stop=max(exponential_samples) + 10, num=10000)
